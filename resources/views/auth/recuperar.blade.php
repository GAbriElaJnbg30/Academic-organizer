...
<body>
    ...
    <div class="container-dos">
        ...
        <div class="input-container-dos">
            <form action="{{ route('restablecer.cuenta') }}" method="POST" class="form-restablecer">
                @csrf
                <input type="hidden" name="token" value="{{ $token }}">

                <div class="input-contraseña">
                    <input type="password" name="password" id="password" placeholder="Ingrese una nueva contraseña" required maxlength="15">
                    <span><i class="fas fa-lock icon-contraseña"></i></span>
                    <i class="fas fa-eye-slash icon-eye" id="togglePassword"></i>
                    @error('password')
                ...
                <div class="input-contraseña">
                    <input type="password" name="password_confirmation" placeholder="Confirme la nueva contraseña" id="password_confirmation" required maxlength="15">
                    <span><i class="fas fa-lock icon-contraseña"></i></span>
                    <i class="fas fa-eye-slash icon-eye" id="togglePasswordConfirmation"></i>
                    @error('password_confirmation')
                        <div class="error-mess">{{ $message }}</div>
                    @enderror
                </div>
                <button type="submit" class="reestablecer">Restablecer</button>
            </form>
        </div>
    </div>
    ...
    const icon = this;
    // Cambiar entre mostrar u ocultar la contraseña
    if (passwordField.type === "password") {
        passwordField.type = "text"; // Mostrar la contraseña
        icon.classList.remove("fa-eye-slash");
        icon.classList.add("fa-eye"); // Cambiar el icono
    } else {
        passwordField.type = "password"; // Ocultar la contraseña
        icon.classList.remove("fa-eye");
        icon.classList.add("fa-eye-slash"); // Cambiar el icono
    }
});
...
document.getElementById("togglePasswordConfirmation").addEventListener("click", function() {
    ...
    const icon = this;
    // Cambiar entre mostrar u ocultar la contraseña de confirmación
    if (passwordConfirmationField.type === "password") {
        passwordConfirmationField.type = "text"; // Mostrar la contraseña
        icon.classList.remove("fa-eye-slash");
        icon.classList.add("fa-eye"); // Cambiar el icono
    } else {
        passwordConfirmationField.type = "password"; // Ocultar la contraseña
        icon.classList.remove("fa-eye");
        icon.classList.add("fa-eye-slash"); // Cambiar el icono
    }
});
